...
@section('content')
@php
    $selectedMenus = $waste_menus->menus->pluck('menu_id')->toArray();
    $selectedQuantities = $waste_menus->menus->pluck('pivot.jumlah')->toArray();
@endphp
<h2><b><center>Ubah Waste Menu</center></b></h2><br>
<form action="{{ route('waste_menus.update', ['waste_menu' => $waste_menus->waste_menu_id]) }}" method="POST" enctype="multipart/form-data">
    @method('PATCH')
    @csrf
            ...
            <div class="form-group">
                ...
                <input type="date" class="form-control @error('tanggal') is-invalid @enderror" id="tanggal" name="tanggal" value="{{ old('tanggal', $waste_menus->tanggal) }}" readonly>
                @error('tanggal')
